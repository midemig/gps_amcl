...
class AmclNode
{
  ...
  private:
    ...
    ros::Publisher particlecloud_pub_2;
        ...
        first_reconfigure_call_(true)
{
  ...
  particlecloud_pub_2 = nh_.advertise<geometry_msgs::PoseArray>("particlecloud_2", 2, true);
  ...
  gps_sub_ = nh_.subscribe(gps_topic_, 1, &AmclNode::gpsPoseReceived, this);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(resampled || force_publication)
  {
    ...
    geometry_msgs::PoseArray cloud_msg2;
    cloud_msg2.header.stamp = ros::Time::now();
    cloud_msg2.header.frame_id = global_frame_id_;
    cloud_msg2.poses.resize(pf_->sets[pf_->current_set].cluster_count);
        ...
        hyp_count < pf_->sets[pf_->current_set].cluster_count; hyp_count++)
    {
      ...
      cloud_msg2.poses[hyp_count].position.x = hyps[hyp_count].pf_pose_mean.v[0];
      cloud_msg2.poses[hyp_count].position.y = hyps[hyp_count].pf_pose_mean.v[1];
      cloud_msg2.poses[hyp_count].position.z = 0;
      tf2::Quaternion q;
      q.setRPY(0, 0, hyps[hyp_count].pf_pose_mean.v[2]);
      tf2::convert(q, cloud_msg2.poses[hyp_count].orientation);
    }

    particlecloud_pub_2.publish(cloud_msg2);
...
///
void
AmclNode::gpsPoseReceived(const nav_msgs::Odometry& msg)
{
  tf::Quaternion q(
